...
- **data/VAL-05 / cross/CV-02 (phase 2):** the internal `TraversalLimitError` (thrown by the bounded walker behind the depth, size and circular guards) is now the `@zudojs/errors` class (a `BaseError`, not exposed). The public guards translate it exactly as before.
