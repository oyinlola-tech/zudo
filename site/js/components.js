...
(function () {
  ...
  var SEARCH_INDEX = [
    ...
    { g: 'Learn', t: "Iterables and iterators (Advanced JavaScript 6)", p: '/learn/js-iterators', e: "Learn the protocol behind for...of, spread and destructuring, then write your own iterables: a number range and an API paginator that fetches pages lazily." },
    { g: 'Learn', t: "Generators (Advanced JavaScript 7)", p: '/learn/js-generators', e: "Write iterators as ordinary loops with function* and yield, then stream order lines lazily from a paged API into a CSV export, with cleanup that always runs." },
    { g: 'Learn', t: "Symbols (Advanced JavaScript 8)", p: '/learn/js-symbols', e: "Use symbols as keys that never clash, plug your objects into the language with well-known symbols, and build a Money type that refuses to add naira to dollars." },
    { g: 'Learn', t: "Promises in depth (Advanced JavaScript 9)", p: '/learn/js-promises', e: "See exactly what a promise guarantees, how chains pass values and errors along, how thenables work, and how to turn a callback SDK into promises safely." },
    { g: 'Learn', t: "Combining promises (Advanced JavaScript 10)", p: '/learn/js-promise-combinators', e: "Run independent work in parallel with Promise.all, allSettled, race and any, put a time limit on a slow provider, and choose sequential, parallel or batched." },
    { g: 'Learn', t: "The event loop (Advanced JavaScript 11)", p: '/learn/js-event-loop', e: "Build an exact model of the call stack, task queue and microtask queue, solve ordering puzzles with it, and keep long jobs from starving timers and requests." },
    ...
    { g: 'Learn', t: "npm and packages (JavaScript in the browser and on the server 5)", p: '/learn/npm-packages', e: "Install, update and remove packages with npm, read package.json and package-lock.json, understand version ranges, write npm scripts, share code with workspaces, see what publishing would send, and keep the packages you install safe." },
    { g: 'Learn', t: "An HTTP server with no framework (JavaScript in the browser and on the server 6)", p: '/learn/node-http', e: "Build a web server with nothing but Node.js's node:http module. Read the method, path, headers, query and body of each request, send JSON back with the right status code, and route requests by hand." },
    { g: 'Learn', t: "Build a plain Node.js Task API (JavaScript in the browser and on the server 7)", p: '/learn/node-task-api', e: "Finish a complete Task API with no framework, split into modules, with middleware, centralized error handling, API-key protection and configuration from the environment. Then look honestly at what hurts when it grows to 20,000 lines." },
    { g: 'Learn', t: "The DOM (JavaScript in the browser and on the server 8)", p: '/learn/browser-dom', e: "Read and change a web page from JavaScript: find, create, update and remove elements safely, avoid innerHTML XSS, and build a task list UI rendered from data." },
    { g: 'Learn', t: "The debugging method (JavaScript in the browser and on the server 9)", p: '/learn/debug-method', e: "Find bugs with a repeatable method (observe, reproduce, isolate, hypothesize, test, fix, verify), read stack traces, log well and shrink a bug to a minimal repro." },
    { g: 'Learn', t: "Git and GitHub (JavaScript in the browser and on the server 10)", p: '/learn/git', e: "Track every change to your code with Git, work on branches, merge them and resolve a real conflict, keep node_modules and secrets out of your repository, and share your work through GitHub with remotes and pull requests." },
    ...
    { g: 'Learn', t: "Dependency injection with @zudojs/container (ZudoJS fundamentals 8)", p: '/learn/zudo-container', e: "Let a container build and share your services. Learn tokens, class, factory and value providers, the singleton, scoped and transient lifetimes, circular dependency detection, disposal, and swapping real services for fakes in tests." },
    { g: 'Learn', t: "Routes, requests and responses (ZudoJS fundamentals 9)", p: '/learn/zudo-http', e: "Serve the Task API over HTTP with @zudojs/http. Start a server, add routes with parameters, read query strings, headers, cookies and JSON bodies safely, and answer with the right status codes." },
    { g: 'Learn', t: "Middleware, CORS, security headers and graceful shutdown (ZudoJS fundamentals 10)", p: '/learn/zudo-middleware', e: "Wrap every Task API route in middleware. Learn how a middleware pipeline runs, then add security headers, a CORS allow-list, rate limits with @zudojs/security, and a graceful shutdown that lets requests in progress finish." },
    { g: 'Learn', t: "Configuration (ZudoJS fundamentals 11)", p: '/learn/zudo-config', e: "Keep settings out of the code with @zudojs/config. Layer defaults, a config file and environment variables by priority, validate the result with a schema, keep secrets out of logs, and make the Task API's generated configuration stricter in production than in development." },
    { g: 'Learn', t: "Schemas and validation in depth (ZudoJS fundamentals 12)", p: '/learn/zudo-validation', e: "Validate everything that crosses the Task API's boundary with @zudojs/schema. Read issues, coerce query strings, transform and refine values, block unknown fields, validate requests in routes and responses on the way out, and guard against hostile JSON with @zudojs/validation." },
    { g: 'Learn', t: "The ZudoJS error system (ZudoJS fundamentals 13)", p: '/learn/zudo-errors', e: "Handle failures the ZudoJS way. Tell expected errors from bugs, use and extend the error classes in @zudojs/errors, choose codes and categories, serialize errors safely for clients and fully for logs, and turn every error in the Task API into the right HTTP response." },
