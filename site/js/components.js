...
(function () {
  ...
  var SEARCH_INDEX = [
    ...
    { g: 'Learn', t: "Problem workshop: beginner (Programming thinking 15)", p: '/learn/solve-beginner', e: "Solve ten everyday problems, from even or odd to a tax calculator, by reasoning about inputs, outputs and edge cases first, then coding and testing each one." },
    { g: 'Learn', t: "Problem workshop: intermediate (Programming thinking 16)", p: '/learn/solve-intermediate', e: "Solve eight problems on lists of real data, from duplicate payments to stock levels, reasoning about edge cases and the amount of work first, then coding and testing." },
    { g: 'Learn', t: "Why doesn't this work? (Programming thinking 17)", p: '/learn/solve-broken', e: "Diagnose broken algorithms that run without errors but give wrong answers, using trace tables, predictions and boundary tests, then fix each root cause." },
    ...
    { g: 'Learn', t: "Objects in depth (JavaScript fundamentals 11)", p: '/learn/js-objects-deep', e: "Control exactly how an object behaves: lock its shape, validate writes with setters, hide fields, copy it safely and compare it, by building a bank account that cannot be broken." },
    ...
    { g: 'Learn', t: "Recursion (JavaScript fundamentals 15)", p: '/learn/js-recursion', e: "Solve problems on nested data of any depth by letting a function call itself: folder sizes, comment threads and org charts, then make it safe against deep and circular data." },
    ...
    { g: 'Learn', t: "Trees and binary search trees (Algorithms and data structures 4)", p: '/learn/dsa-trees', e: "Turn flat category rows into a tree, walk it four ways, then build, test and balance a binary search tree of orders, with the cost of every operation." },
    { g: 'Learn', t: "Heaps and priority queues (Algorithms and data structures 5)", p: '/learn/dsa-heaps', e: "Build a binary heap on a plain array, use it as a priority job queue, then sort with it, find the top-k products and merge sorted order lists." },
    { g: 'Learn', t: "Graphs and topological sort (Algorithms and data structures 6)", p: '/learn/dsa-graphs', e: "Model packages, roads between cities and followers as graphs, store them as adjacency lists, and compute a safe install order with topological sort." },
    { g: 'Learn', t: "Tries and autocomplete (Algorithms and data structures 7)", p: '/learn/dsa-tries', e: "Build a prefix tree for product search: autocomplete as the user types, count matches per prefix, delete safely, and weigh the memory it costs." },
    { g: 'Learn', t: "this in depth (Advanced JavaScript 1)", p: '/learn/js-this', e: "Predict the value of this in every kind of call, fix lost this with call, apply, bind and arrow functions, and build your own bind and bindAll to see how they work." },
    { g: 'Learn', t: "Prototypes in depth (Advanced JavaScript 2)", p: '/learn/js-prototypes', e: "Follow the prototype chain step by step, build objects with Object.create and constructor functions, rewrite new, instanceof and extends yourself, and see exactly what a class becomes." },
    ...
    { g: 'Learn', t: "Union types in depth (TypeScript 7)", p: '/learn/ts-unions', e: "Model real states with literal and discriminated unions, return typed results instead of throwing, combine shapes with intersections, and avoid the common union mistakes." },
    { g: 'Learn', t: "Type aliases and interfaces (TypeScript 8)", p: '/learn/ts-aliases-interfaces', e: "Name the types of a banking domain once and reuse them, extend them safely, understand declaration merging and recursive types, and learn the real differences between type and interface." },
    { g: 'Learn', t: "Narrowing (TypeScript 9)", p: '/learn/ts-narrowing', e: "Turn wide union and unknown types into precise ones with typeof, truthiness, equality, in, instanceof, discriminants, type guards and assertion functions, and learn where narrowing lies." },
    { g: 'Learn', t: "Type assertions (TypeScript 10)", p: '/learn/ts-assertions', e: "Learn what as, angle-bracket assertions, as unknown as and the non-null ! really do, see why none of them check or convert anything, and replace them with safer tools." },
    { g: 'Learn', t: "Enums and their alternatives (TypeScript 11)", p: '/learn/ts-enums', e: "Write numeric, string and const enums, read the exact JavaScript tsc emits for each, learn the pitfalls and erasableSyntaxOnly, and replace enums with unions and as const objects." },
    { g: 'Learn', t: "Tuples (TypeScript 12)", p: '/learn/ts-tuples', e: "Type fixed-shape arrays with tuples - optional, rest and named elements, readonly tuples, tuple inference and const type parameters - and return tuples from functions the way useState does." },
    { g: 'Learn', t: "Generics (TypeScript 13)", p: '/learn/ts-generics', e: "Write one function, interface or class that works for many types without losing type safety, set rules with constraints and keyof, give type parameters defaults, and build a Result type and a generic repository." },
    { g: 'Learn', t: "Advanced and utility types (TypeScript 14)", p: '/learn/ts-advanced', e: "Build new types from existing ones with keyof, typeof, indexed access, mapped, conditional and template literal types, and use the built-in utility types (Partial, Pick, Omit, Record, ReturnType, Awaited and more) to keep one source of truth." },
    { g: 'Learn', t: "Classes in TypeScript (TypeScript 15)", p: '/learn/ts-classes', e: "Declare typed class properties, use parameter properties and access modifiers, compare private with #private, write abstract classes and classes that implement interfaces, and wire classes together with constructor injection." },
    { g: 'Learn', t: "Modules in TypeScript (TypeScript 16)", p: '/learn/ts-modules', e: "Split a TypeScript project into files, import types with import type, see why verbatimModuleSyntax exists, write .js in import paths, and understand how NodeNext resolution, \"type\" - \"module\" and the \"exports\" field fit together." },
    { g: 'Learn', t: "TypeScript and JavaScript together (TypeScript 17)", p: '/learn/ts-runtime', e: "See exactly what TypeScript becomes when it runs, why types cannot check outside data, and how to close that gap by hand with type guards, assertion functions and a validator that returns a Result." },
    ...
    { g: 'Learn', t: "The application runtime and lifecycle (ZudoJS fundamentals 5)", p: '/learn/zudo-runtime', e: "Learn how the ZudoJS runtime starts the parts of your application in dependency order, rolls back when startup fails, reports readiness, and shuts everything down gracefully." },
    { g: 'Learn', t: "Dependency injection with @zudojs/container (ZudoJS fundamentals 6)", p: '/learn/zudo-container', e: "Let a container build and share your services. Learn tokens, class, factory and value providers, the singleton, scoped and transient lifetimes, circular dependency detection, disposal, and swapping real services for fakes in tests." },
    { g: 'Learn', t: "Routes, requests and responses (ZudoJS fundamentals 7)", p: '/learn/zudo-http', e: "Serve the Task API over HTTP with @zudojs/http. Start a server, add routes with parameters, read query strings, headers, cookies and JSON bodies safely, and answer with the right status codes." },
    { g: 'Learn', t: "Middleware, CORS, security headers and graceful shutdown (ZudoJS fundamentals 8)", p: '/learn/zudo-middleware', e: "Wrap every Task API route in middleware. Learn how a middleware pipeline runs, then add security headers, a CORS allow-list, rate limits with @zudojs/security, and a graceful shutdown that lets requests in progress finish." },
    { g: 'Learn', t: "Configuration (ZudoJS fundamentals 9)", p: '/learn/zudo-config', e: "Keep settings out of the code with @zudojs/config. Layer defaults, a config file and environment variables by priority, validate the result with a schema, keep secrets out of logs, and make the Task API's generated configuration stricter in production than in development." },
    ...
    { g: 'Learn', t: "The ZudoJS error system (ZudoJS fundamentals 11)", p: '/learn/zudo-errors', e: "Handle failures the ZudoJS way. Tell expected errors from bugs, use and extend the error classes in @zudojs/errors, choose codes and categories, serialize errors safely for clients and fully for logs, and turn every error in the Task API into the right HTTP response." },
