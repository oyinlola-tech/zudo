...
(function () {
  ...
  var SEARCH_INDEX = [
    ...
    { g: 'Learn', t: "31. Why TypeScript exists", p: '/learn/ts-setup', e: "See a bug that JavaScript runs without complaint, install TypeScript, write a tsconfig.json, and learn the three ways to run TypeScript on Node.js 24." },
    { g: 'Learn', t: "32. Basic types", p: '/learn/ts-types', e: "The everyday types of TypeScript - strings, numbers, booleans, arrays, tuples and object types - plus any vs unknown, null and undefined under strict mode, void, and never for exhaustive checks." },
    { g: 'Learn', t: "33. Typing functions", p: '/learn/ts-functions', e: "Give functions parameter and return types, use optional, default and rest parameters, describe functions and callbacks as types, type async functions with Promise, and meet overloads." },
    { g: 'Learn', t: "34. Interfaces, unions and literal types", p: '/learn/ts-objects', e: "Name object shapes with interfaces and type aliases, mark properties optional or readonly, extend and combine shapes, and model data that can take several forms with unions, literal types and discriminated unions." },
    { g: 'Learn', t: "35. Generics", p: '/learn/ts-generics', e: "Write one function, interface or class that works for many types without losing type safety, set rules with constraints and keyof, give type parameters defaults, and build a Result type and a generic repository." },
    { g: 'Learn', t: "36. Advanced and utility types", p: '/learn/ts-advanced', e: "Build new types from existing ones with keyof, typeof, indexed access, mapped, conditional and template literal types, and use the built-in utility types (Partial, Pick, Omit, Record, ReturnType, Awaited and more) to keep one source of truth." },
    { g: 'Learn', t: "37. Classes in TypeScript", p: '/learn/ts-classes', e: "Declare typed class properties, use parameter properties and access modifiers, compare private with #private, write abstract classes and classes that implement interfaces, and wire classes together with constructor injection." },
    { g: 'Learn', t: "38. Modules in TypeScript", p: '/learn/ts-modules', e: "Split a TypeScript project into files, import types with import type, see why verbatimModuleSyntax exists, write .js in import paths, and understand how NodeNext resolution, \"type\" - \"module\" and the \"exports\" field fit together." },
    { g: 'Learn', t: "39. TypeScript and JavaScript together", p: '/learn/ts-runtime', e: "See exactly what TypeScript becomes when it runs, why types cannot check outside data, and how to close that gap by hand with type guards, assertion functions and a validator that returns a Result." },
    { g: 'Learn', t: "40. \"BookStore API: HTTP and routing\"", p: '/learn/bookstore-http', e: "Start a TypeScript backend with no framework at all. Set up the folder, read configuration from the environment, write a small typed router, JSON helpers and error responses, and serve books and authors over HTTP." },
    { g: 'Learn', t: "41. \"BookStore API: validation and PostgreSQL\"", p: '/learn/bookstore-data', e: "Check every request body by hand with type guards, move the BookStore data into PostgreSQL with PGlite, write repositories with parameterized SQL, and map typed errors to 400, 404 and 409." },
    { g: 'Learn', t: "42. \"BookStore API: authentication and tests\"", p: '/learn/bookstore-auth', e: "Add users to the BookStore. Hash passwords with scrypt, issue signed log-in tokens with a secret from the environment, protect the order routes, test it all with node:test, and then look honestly at what this hand-built backend now gets wrong." },
    { g: 'Learn', t: "43. Your first Zudo code", p: '/learn/zudo-first-code', e: "Install your first ZudoJS packages, check untrusted data at runtime with @zudojs/schema, and report failures with the ready-made errors in @zudojs/errors." },
    { g: 'Learn', t: "44. Create the Task API project", p: '/learn/zudo-create-project', e: "Install the ZudoJS command-line tool, create the Task API project, start its server, and find your way around every file it generated." },
    { g: 'Learn', t: "45. The application runtime and lifecycle", p: '/learn/zudo-runtime', e: "Learn how the ZudoJS runtime starts the parts of your application in dependency order, rolls back when startup fails, reports readiness, and shuts everything down gracefully." },
    { g: 'Learn', t: "46. Dependency injection with @zudojs/container", p: '/learn/zudo-container', e: "Let a container build and share your services. Learn tokens, class, factory and value providers, the singleton, scoped and transient lifetimes, circular dependency detection, disposal, and swapping real services for fakes in tests." },
    { g: 'Learn', t: "47. Routes, requests and responses", p: '/learn/zudo-http', e: "Serve the Task API over HTTP with @zudojs/http. Start a server, add routes with parameters, read query strings, headers, cookies and JSON bodies safely, and answer with the right status codes." },
    { g: 'Learn', t: "48. Databases with @zudojs/database", p: '/learn/zudo-database', e: "Connect the Task API to PostgreSQL through @zudojs/database, with migrations, seeds, a repository, a query builder, pagination, transactions and health checks, all running against real PostgreSQL." },
    { g: 'Learn', t: "49. Storage abstractions", p: '/learn/zudo-storage', e: "Use @zudojs/storage's driver-independent contracts for databases, files, serialization, locks, connection pools and start-up and shutdown, with a PostgreSQL adapter and local file storage for the Task API." },
    { g: 'Learn', t: "50. Authentication", p: '/learn/zudo-auth', e: "Let users log in to the Task API. Hash passwords, issue and check JWTs, keep server-side sessions with a real logout, protect routes in @zudojs/http, and stop password guessing with lockouts and rate limits, using @zudojs/auth and @zudojs/crypto." },
    { g: 'Learn', t: "51. Sign in with OAuth", p: '/learn/zudo-oauth', e: "Add \"Sign in with Google\" to the Task API with @zudojs/auth-oauth. Learn the OAuth 2 authorization code flow, state and PKCE, the provider presets, and the callback, all tested offline against a stand-in provider." },
    { g: 'Learn', t: "52. Permissions", p: '/learn/zudo-permissions', e: "Decide what each logged-in user may do with @zudojs/permissions. Roles, resource:action permissions, wildcards, role hierarchy, owner rules, deny rules, policies and explain mode, and the mistakes that let a normal user become an admin." },
    { g: 'Learn', t: "53. Security for every public API", p: '/learn/zudo-security', e: "Protect the Task API from the open internet with @zudojs/security and the security helpers in @zudojs/http. Rate limiting, CORS, CSRF, security headers, HSTS and CSP, secure cookies, body limits, SSRF protection and input checks, each shown blocking a real attack." },
    { g: 'Learn', t: "54. Caching", p: '/learn/zudo-cache', e: "Keep copies of slow results so the Task API can answer again fast. Learn keys, TTL, namespaces, tags and invalidation, stampede protection, locks and cache metrics with @zudojs/cache." },
    { g: 'Learn', t: "55. Events", p: '/learn/zudo-events', e: "Announce that something happened and let other parts of the Task API react, without the code that announces it knowing who listens. Handlers, wildcards, priorities, dispatch modes, middleware and handler errors with @zudojs/events." },
    { g: 'Learn', t: "56. Messaging", p: '/learn/zudo-messaging', e: "Ask another part of the Task API to do something and get an answer back, without importing it. Messages, handlers, middleware, correlation and causation ids, timeouts and cancellation with @zudojs/messaging." },
    { g: 'Learn', t: "57. Commands and queries (CQRS)", p: '/learn/zudo-cqrs', e: "Split the Task API's operations into commands that change data and queries that only read it. Command and query buses, handlers, middleware, execution context, results, and when CQRS is worth it, with @zudojs/cqrs." },
    { g: 'Learn', t: "58. Background jobs", p: '/learn/zudo-queue', e: "Move slow and unreliable work out of the request and into a job queue. Queues, jobs, processors, workers, concurrency, retries with exponential backoff, delayed jobs, dead letters and graceful shutdown with @zudojs/queue." },
    { g: 'Learn', t: "59. Calling services with RPC", p: '/learn/zudo-rpc', e: "Call a function that runs in another service as if it were local. Define procedures with @zudojs/rpc, call them through a client and a transport, and handle validation, errors, identity, timeouts and retries." },
    { g: 'Learn', t: "60. One operation, many transports", p: '/learn/zudo-api', e: "Write your business logic once as an operation with @zudojs/api, then run it from HTTP, RPC and a queue with the same validation, errors, interceptors and timeouts." },
    { g: 'Learn', t: "61. Structured logging", p: '/learn/zudo-logging', e: "Replace console.log with structured log entries that a program can search. Levels, child loggers, request ids, request and error logging, secret redaction and JSON logs for production with @zudojs/logger." },
    { g: 'Learn', t: "62. From monolith to modular monolith", p: '/learn/zudo-modular-monolith', e: "Split one growing application into modules with clear boundaries. Each module has a small public API, owns its data, and talks to the others through that API, events and commands, all inside one deployable app." },
    { g: 'Learn', t: "63. Microservices", p: '/learn/zudo-microservices', e: "Why some teams split one application into many services, why you should not start that way, and how services find and call each other, stay consistent without distributed transactions, survive failures and stay observable." },
    { g: 'Learn', t: "64. Production engineering", p: '/learn/production-engineering', e: "What changes when real users depend on your app. A checklist for configuration, secrets, logs, metrics, traces, health and readiness checks, graceful shutdown, timeouts, rate limits, caching and database performance, with a small runnable proof for each item." },
    /* learn-search:end */
