...
- **You need:** Big O and complexity, Arrays and strings under the hood, Classes and Asynchronous JavaScript
...
A stack needs four operations, all O(1): `push` (add on top), `pop` (remove the top and return it), `peek` (look at the top without removing it) and `size`. A JavaScript array already does this at its end, because `push` and `pop` never move other items (see [Arrays and strings under the hood](https://zudojs.oyinlola.site/learn/dsa-arrays-strings#insert-delete)). Wrapping the array in a class hides the other array methods, so no one can reach into the middle of your stack by accident:
...
It is correct, and for a print queue in one office it is fine. But `shift` moves every remaining item one place (O(n)), so a queue that holds tens of thousands of jobs pays for every dequeue with a full pass over the array. You measured this in [Arrays and strings under the hood](https://zudojs.oyinlola.site/learn/dsa-arrays-strings#shift). The fix is to stop moving items. There are three standard ways.
