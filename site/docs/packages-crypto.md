...
Cryptographic failures throw `CryptoError` from `@zudojs/errors`, re-exported here. It carries a stable `code` such as `ERR_CRYPTO_CIPHER`, an `operation`, and the original Node error as `cause`. Unsafe cryptographic parameters are `CryptoError`s too: a short HMAC key (`ERR_CRYPTO_HASH`), an out-of-range scrypt or PBKDF2 work factor (`ERR_CRYPTO_DERIVATION`, or `ERR_CRYPTO_HASH` for `hashPassword`), a bad key length (`ERR_CRYPTO_KEY`), and a password outside the length bounds (`ERR_CRYPTO_HASH` with `statusCode: 400` and `expose: true`). Only argument-shape mistakes (a key that is not a `Uint8Array`, an unknown algorithm name, a non-integer length) still throw `TypeError` or `RangeError`.
...
- **Passing a 4-byte HMAC key.** `hmac` throws a `CryptoError` with code `ERR_CRYPTO_HASH`: "HMAC key must be at least 16 bytes, received 4." Fix: use 16 to 64 random bytes. A string key throws `TypeError`: encode it first with `new TextEncoder().encode(secret)`.
...
Every name `@zudojs/crypto` exports from its package root at v1.4.0 — **260** in total, generated from the package’s own entry point rather than written by hand. The sections above explain the ones you reach for most; this is the exhaustive list, so nothing shipped is undocumented. Names not covered above are typically internal helpers and supporting types.

**Show all 260 exports**
