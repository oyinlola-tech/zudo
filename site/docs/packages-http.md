...
| --- | --- | --- |
| `summary / description / operationId / tags / deprecated` | Copied onto the operation | Router groups can set `openapi` defaults: tags are merged, every other field is overridden by the route. |
...
| `responses` | Keyed by status, e.g. `"200"`, `"4XX"`, `"default"` | `{ schema }` or `{ description }`; the description defaults to the reason phrase. |
| `security` | Who may call it | `[]` marks the route public, overriding document-wide security. |
| `false` or `{ hidden: true }` | Leave the route out | Use it for health checks and internal endpoints. |
...
| `origin` | Origin for `request.url` when the request has no usable host | Default `http://localhost`. |
| `name / openapi` | Route name and documentation | `openapi` defaults to `false`: a mounted handler documents its own operations. |

`toWebRequest(context)` does the request conversion on its own, if you want to call a fetch handler from inside a normal route.
...
| `mountFetchHandler(router, basePath, handler, options?)` | Serves a `Request` → `Response` handler under a path | Returns an unmount function. See [Mounting](#mounting-fetch-handlers). |
| `toWebRequest(context, options?)` | Converts a request context into a web `Request` | What `mountFetchHandler` uses. |
...
Every name `@zudojs/http` exports from its package root at v1.3.0 — **1730** in total, generated from the package’s own entry point rather than written by hand. The sections above explain the ones you reach for most; this is the exhaustive list, so nothing shipped is undocumented. Names not covered above are typically internal helpers and supporting types.

**Show all 1730 exports**
