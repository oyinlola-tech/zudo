...
> **Watch out**
>
> The published `0.1.0` release of this package shipped without `dist/index.js`, so `import { ... } from "@zudojs/http"` fails on that version. The 1.0.0 sources fix it; until 1.0.0 is published, install from the repository rather than the registry.
...
A handler may also return a plain object. The adapter turns it into a `200` JSON response. Returning nothing sends an empty `200`.
...
| `getQuery(name)` | One query value, an array for repeats, or `undefined` | Also `query` for the whole object. |
...
| `cookie(name, value, options?)` | Adds a `Set-Cookie` header | Options: `httpOnly`, `secure`, `sameSite`, `maxAge`, `path`. |
...
| `trustProxy` | Whether to believe `X-Forwarded-*` headers | `false` (default), `true`, a hop count, or a list of proxy IPs. |
...
> **Watch out**
>
> The built-in factories `createCorsMiddleware`, `createSecurityMiddleware`, and `createTimingMiddleware` return a plain-object copy of the response instead of the `HttpResponseContext`. With the Node adapter that copy loses its status and body. Until this is fixed, write these few lines yourself as shown in [CORS & security headers](#cors-and-security-headers).
...
| `credentials` | Allow cookies and `Authorization` | Default `false`. |
...
- **Using the built-in `createCorsMiddleware` / `createSecurityMiddleware` with the Node adapter.** They return a plain object that loses the status and body. Use `evaluateCors` and `createRecommendedSecurityHeaders` inside your own middleware instead.
