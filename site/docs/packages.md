...
No dependencies](https://zudojs.oyinlola.site/docs/packages-errors.md) [@zudojs/types v1.2.0

Shared type guards, utility types, and type converters for the Zudo framework.

No dependencies](https://zudojs.oyinlola.site/docs/packages-types.md) [@zudojs/constants v1.1.2

Shared constants, enums, and type-safe literals for the Zudo framework.

depends on: errors](https://zudojs.oyinlola.site/docs/packages-constants.md) [@zudojs/container v1.2.0

Token-based dependency injection container for managing application dependencies and service lifetimes.

depends on: errors](https://zudojs.oyinlola.site/docs/packages-container.md) [@zudojs/logger v1.4.0

Structured logging with transports, log levels, and context propagation for Zudo applications.

depends on: errors](https://zudojs.oyinlola.site/docs/packages-logger.md) [@zudojs/crypto v1.3.1

Cryptographic primitives for hashing, encryption, tokens, and secure random generation.

depends on: constants, errors](https://zudojs.oyinlola.site/docs/packages-crypto.md) [@zudojs/validation v1.1.0

Schema validation with Zod integration, constraints, parsers, composers, and depth/size checks.

depends on: errors, constants, types](https://zudojs.oyinlola.site/docs/packages-validation.md) [@zudojs/schema v1.2.0

Type-safe schema definition, parsing, and validation engine for data contracts.

depends on: errors, constants, types](https://zudojs.oyinlola.site/docs/packages-schema.md) [@zudojs/config v1.3.0

Layered configuration management with multiple sources, validation, and environment-specific overrides.

depends on: errors, constants](https://zudojs.oyinlola.site/docs/packages-config.md) [@zudojs/middleware v1.1.0

Composable middleware pipeline with composition, timing, error handling, and context propagation.

depends on: errors](https://zudojs.oyinlola.site/docs/packages-middleware.md) [@zudojs/serialization v1.2.0
...
depends on: errors, constants, middleware](https://zudojs.oyinlola.site/docs/packages-events.md) [@zudojs/messaging v1.2.0

In-process message bus infrastructure with handlers, middleware, and publish/subscribe patterns.

depends on: errors, constants, middleware](https://zudojs.oyinlola.site/docs/packages-messaging.md) [@zudojs/lifecycle v1.2.1

Application and component lifecycle orchestration with state machine, dependency ordering, graceful shutdown.

depends on: errors, constants](https://zudojs.oyinlola.site/docs/packages-lifecycle.md) [@zudojs/transactions v1.2.0

Transaction lifecycle and coordination with state machine, AsyncLocalStorage, savepoints, and hooks.

depends on: errors](https://zudojs.oyinlola.site/docs/packages-transactions.md) [@zudojs/cache v1.2.0

Caching primitives, abstractions, and adapters for the Zudo framework.

depends on: errors, constants, types, serialization](https://zudojs.oyinlola.site/docs/packages-cache.md) [@zudojs/storage v1.2.0

Storage abstractions including database, object storage, repository, serialization, locking, and lifecycle.

depends on: errors, constants, types, serialization](https://zudojs.oyinlola.site/docs/packages-storage.md) [@zudojs/queue v1.4.0

Background job and asynchronous task infrastructure with in-memory and adapter-based queue implementations.

depends on: errors, constants, serialization](https://zudojs.oyinlola.site/docs/packages-queue.md) [@zudojs/scheduler v1.2.0

Scheduled task and job infrastructure with cron-like scheduling, persistence, and worker management.

depends on: errors, constants, types](https://zudojs.oyinlola.site/docs/packages-scheduler.md) [@zudojs/adapters v1.2.1

Boundary layer between Zudo and external platforms with adapter contracts, registry, and transport abstractions.

depends on: errors](https://zudojs.oyinlola.site/docs/packages-adapters.md) [@zudojs/database v1.3.0

Database abstraction layer with clients, repositories, transactions, and query building.

depends on: errors, types, logger](https://zudojs.oyinlola.site/docs/packages-database.md) [@zudojs/observability v1.2.0

Structured logging, metrics, tracing, context propagation, and exporters for Zudo applications.

depends on: errors, logger](https://zudojs.oyinlola.site/docs/packages-observability.md)
...
depends on: errors, constants](https://zudojs.oyinlola.site/docs/packages-core.md) [@zudojs/cqrs v1.2.0

CQRS primitives for separating read and write operations.

depends on: errors, events, middleware](https://zudojs.oyinlola.site/docs/packages-cqrs.md) [@zudojs/auth v1.3.0

Authentication and authorization services — JWT tokens, sessions, RBAC, and password hashing.

depends on: errors, constants, types, crypto, permissions](https://zudojs.oyinlola.site/docs/packages-auth.md) [@zudojs/auth-oauth v1.2.2

Sign in with Google, GitHub, Microsoft, Apple or Discord — authorization URLs, PKCE, state checks, and code exchange.

depends on: errors, security](https://zudojs.oyinlola.site/docs/packages-auth-oauth.md) [@zudojs/runtime v1.3.0

Application lifecycle orchestrator with dependency ordering, rollback, signals, and readiness checks.

depends on: errors, constants, container, core, events, logger](https://zudojs.oyinlola.site/docs/packages-runtime.md) [@zudojs/api v1.2.0

Application-facing API layer — operation definitions, execution context, interceptors, and result types.

depends on: errors, types, openapi, queue, rpc, schema, security, serialization](https://zudojs.oyinlola.site/docs/packages-api.md) [@zudojs/rpc v1.4.0

Remote procedure call infrastructure for Zudo applications.

depends on: errors, constants, types, schema, security, serialization](https://zudojs.oyinlola.site/docs/packages-rpc.md) [@zudojs/openapi v1.5.0

OpenAPI 3.1 specification generation, validation, and SDK generation for Zudo applications.

depends on: errors, constants](https://zudojs.oyinlola.site/docs/packages-openapi.md)
...
depends on: errors, crypto, logger, middleware, openapi, security](https://zudojs.oyinlola.site/docs/packages-http.md) [zudojs-cli v2.1.0
...
depends on: errors, constants](https://zudojs.oyinlola.site/docs/packages-security.md) [@zudojs/permissions v1.4.0

Generic authorization engine with RBAC, ABAC, resource authorization, wildcards, role hierarchy, and policies.

depends on: errors, middleware](https://zudojs.oyinlola.site/docs/packages-permissions.md) [@zudojs/crypto Foundation
...
depends on: errors, constants, middleware](https://zudojs.oyinlola.site/docs/packages-tenancy.md) [@zudojs/feature-flags v1.4.0

Feature flag system with deterministic rollouts, rule engine, providers, variants, and evaluation context.

depends on: errors, types](https://zudojs.oyinlola.site/docs/packages-feature-flags.md) [@zudojs/plugins v1.3.0

Plugin system for extending Zudo applications with modular capabilities.

depends on: errors](https://zudojs.oyinlola.site/docs/packages-plugins.md) [@zudojs/docs v1.0.4

Documentation infrastructure with structured document model, registry, validation, navigation, and generation.

depends on: errors](https://zudojs.oyinlola.site/docs/packages-docs.md) [@zudojs/testing v1.2.0

Test helpers, fixtures, mocks, and utilities for testing Zudo applications.

depends on: 14 packages](https://zudojs.oyinlola.site/docs/packages-testing.md)
