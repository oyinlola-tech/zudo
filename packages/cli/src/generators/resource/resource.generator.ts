 ...
 * app's `src/container.ts`. Every precondition is checked before the first
 * write (`resource.checks.ts`); running it twice for the same name fails
 * with the files that already exist, and it never duplicates a registration.
 */
...
import { assertResourceWritable } from "./resource.checks.js";
...
export interface ResourceGenerationResult {
  ...
  /** The Prisma schema a model was appended to, when one was. */
...
export async function generateResource(
  ...
  assertResourceWritable({
    cwd, schematic: options.schematic, names, layout, plan, wiring, edits, force: options.force === true,
  });
  ...
  return {
    ...
    manualSteps,
    ...(prismaSchema === undefined ? {} : { prismaSchema }),
  };
}
